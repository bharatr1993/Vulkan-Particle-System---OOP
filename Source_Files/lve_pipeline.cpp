...
namespace lve {
    ...
    LvePipeline::LvePipeline(
            ...
            bool indirect) : lveDevice{device} {
        createPipelineLayout(descriptorSet);
        createGraphicsPipeline(vertFilepath, fragFilepath, configInfo,indirect);
    }
   ...
   */
    ...
    void LvePipeline::loadModelsIndirect(std::vector<std::string> objPath, std::vector<std::string> filePath) {
        // std::vector<LveModel::Vertex> vertices{ {{0.0f, -0.5f}}, {{0.5f, 0.5f}}, {{-0.5f, 0.5f}} };
        for (auto &obj: objPath)
            lveModel.push_back(std::make_shared<LveModel>(lveDevice, obj, filePath, descriptorSet, true));
    }

    std::vector<VkVertexInputBindingDescription> LvePipeline::getBindingDescriptions(bool indirect) {
        if (indirect) {
    ...
    std::vector<VkVertexInputAttributeDescription> LvePipeline::getAttributeDescriptions(bool indirect) {
...
//Particle Pipeline----------------------------------------------------------
    ...
    void LvePipeline::createGraphicsPipeline(
        ...
        auto bindingDescriptions = getBindingDescriptions(indirect);
        auto attributeDescriptions = getAttributeDescriptions(indirect);
